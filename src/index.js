...
getDocs(colRefJSScores)
  .then((snapshot) => {
    ...
    scores.forEach((entry, index) => {
      const row = document.createElement("tr");
      row.innerHTML = `<td>${index + 1}</td><td>${entry.username}</td><td>${entry.js_score}</td>`;
      leaderboardTable.appendChild(row);
    });
  })
  .catch((err) => {
...
document.addEventListener("DOMContentLoaded", async function () {
  const react_score = document
    .getElementById("reactquizform");
...
getDocs(colRefReactScores)
  .then((snapshot) => {
    ...
    scores.forEach((entry, index) => {
      const row = document.createElement("tr");
      row.innerHTML = `<td>${index + 1}</td><td>${entry.username}</td><td>${entry.react_score}</td>`;
      leaderboardTable.appendChild(row);
    });
  })
  .catch((err) => {
...
document.addEventListener("DOMContentLoaded", async function () {
  const python_score = document
    .getElementById("pythonquizform");
...
getDocs(colRefPythonScores)
  .then((snapshot) => {
    ...
    const leaderboardTable = document.getElementById("leaderboardTablePython");

    scores.forEach((entry, index) => {
      const row = document.createElement("tr");
      row.innerHTML = `<td>${index + 1}</td><td>${entry.username}</td><td>${entry.python_score}</td>`;
      leaderboardTable.appendChild(row);
    });
  })
  .catch((err) => {
...
document.addEventListener("DOMContentLoaded", async function () {
  const django_score = document
    .getElementById("djangoquizform");
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  getDocs(colRefDjangoScores)
    .then((snapshot) => {
      ...
      const leaderboardTable = document.getElementById("leaderboardTableDjango");
  
      scores.forEach((entry, index) => {
        const row = document.createElement("tr");
        row.innerHTML = `<td>${index + 1}</td><td>${entry.username}</td><td>${entry.django_score}</td>`;
        leaderboardTable.appendChild(row);
      });
    })
    .catch((err) => {
...
//java
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    const java_score = document
      .getElementById("javaquizform");
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    getDocs(colRefjavaScores)
      .then((snapshot) => {
        ...
        scores.forEach((entry, index) => {
          const row = document.createElement("tr");
          row.innerHTML = `<td>${index + 1}</td><td>${entry.username}</td><td>${entry.java_score}</td>`;
          leaderboardTable.appendChild(row);
        });
      })
      .catch((err) => {
